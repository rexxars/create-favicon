...
describe('cli', () => {
  ...
  test('should skip existing files by default', async () => {
    ...
    for (const target of targetFiles) {
      ...
      expect(result.stderr).toContain(`File ${target} already exists - skipping`)
    }
  })

  test('should replace existing files with `--overwrite`', async () => {
    ...
    for (const target of targetFiles) {
      ...
      expect(result.stderr).toContain(`File ${target} already exists - replacing`)
    }
  })
